...
$newgroup = "haccgroup$rando";
$newkeyp = "hacckey$rando";
...
setkeypath("../status/");
print "SUCCESS: set keypath: $keypath\n";

setcleanup($docleanup);
print "SUCCESS: set cleanup mode: $cleanup\n";
...
$rc = discover_test_state("hacc");
print "SUCCESS: discovered existing test state...$rc\n";
if ($rc) {
    print "SUCCESS: no previous state discovered, starting anew\n";
    ...
    print "SUCCESS: previous test state discovered, continuing test\n";
}
...
print "SUCCESS: ran command (uname -a) on instance\n";
